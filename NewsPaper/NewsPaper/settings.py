...
EMAIL_HOST_USER = tais922('EMAIL_HOST_USER') # ваше имя пользователя, 
# например если ваша почта user@yandex.ru, то сюда надо писать user, иными словами, это всё то что идёт до собаки
EMAIL_HOST_PASSWORD = tais922('EMAIL_HOST_PASSWORD') # пароль от почты
...
SERVER_EMAIL = tais922('SERVER_EMAIL') # это будет у нас вместо аргумента FROM в массовой рассылке
# --]
...
DEFAULT_FROM_EMAIL = tais922('DEFAULT_FROM_EMAIL + ‘@yandex.ru’')  # здесь указываем уже свою ПОЛНУЮ почту с которой будут отправляться письма 
